...
class BookingController extends Controller
{
    ...
    {
        $request->validate([
            'seats' => ['required', 'array', 'min:10', 'max:40', 'exists:seats,id'],
        ]);
    ...
    if (!$club) {
        ...
        // check if the user has enough balance to book the tickets // CHANGE LATER
        if ($club->balance < $booking->total_price) {
            return back()
